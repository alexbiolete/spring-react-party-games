...
const Dashboard = ({ refreshPage }) => {
  ...
  const deleteUser = (user) => {
    axios.delete(`${dbApiUrl}/user?userId=` + user.id, user);
    refreshPage();
  }

  const deleteRoom = (room) => {
    axios.delete(`${dbApiUrl}/room?roomId=` + room.id, room);
    refreshPage();
  }

  const deleteGame = (game) => {
    axios.delete(`${dbApiUrl}/game?gameId=` + game.id, game);
    refreshPage();
  }
          ...
          <div className="px-4 py-2 m-1 space-y-2 overflow-y-auto">
            {users.map((user) => (
              <div key={user.id} className="flex items-center space-x-3">
                ...
                <div>
                  {user.type !== 'admin' && (
                    <button title="Remove" className="text-red-600 hover:text-red-400 transition ease-in-out duration-500" onClick={() => deleteUser(user)}>
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
          ...
          <div className="px-4 py-2 m-1 space-y-3 overflow-y-auto">
            {rooms.map((room) => (
              <div key={room.id} className="flex items-center space-x-3">
                ...
                <button title="Remove" className="text-red-600 hover:text-red-400 transition ease-in-out duration-500" onClick={() => deleteRoom(room)}>
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                  </svg>
                </button>
          ...
          <div className="px-4 py-2 m-1 space-y-3 overflow-y-auto">
            {games.map((game) => (
              <div className="flex items-center space-x-3">
                ...
                <button title="Remove" className="text-red-600 hover:text-red-400 transition ease-in-out duration-500" onClick={() => deleteGame(game)}>
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                  </svg>
                </button>
